...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  $(document).on('click', '#addMember', function(){
  	  thatChat = thisChat();
  	  if((is_okMap[thatChat.data.id] == "1" )||( masterMap[thatChat.data.id] == localuser)){
  		 add_member(thatChat.data.id,group[thatChat.data.id],thatChat.data.name,is_okMap[thatChat.data.id], masterMap[thatChat.data.id]);
  	  }else {
  		layer.msg('你没有权限进行此操作');
  	  }
  ...
  $(document).on('click', '#deleteMember', function(){
  	      thatChat = thisChat();
	  	  if((is_okMap[thatChat.data.id] == "1" ) || ( masterMap[thatChat.data.id] == localuser)){
	       	delete_member(thatChat.data.id,group[thatChat.data.id],thatChat.data.name);
	  	  }else{
	  		layer.msg('你没有权限进行此操作');
	  	  }
     })
  ...
  var elemTpl = ['<div class="layui-layim-main">'
    ,'<div class="layui-layim-info">'
      ...
      ,'<div class="layui-layim-status">'
        ...
        ,'<ul class="layui-anim layim-menu-box">'
          ...
          ,'<li {{d.mine.status === "hide" ? "class=layim-this" : ""}} layim-event="status" lay-type="hide"><i class="layui-icon">&#xe618;</i><cite class="layui-icon layim-status-hide">&#xe60f;</cite>下线</li>'
        ,'</ul>'
      ,'</div>'
    ...
    ,'<li id="addgroup" ><img src="images/IF/chat/addgroup.png" alt="头像"/><span>新建群聊</span></li>'
      ,'<li>'
    ...
    ,'<ul class="layui-unselect layui-layim-tool">'
      ...
      ,'<li class="layui-icon layim-tool-msgVoice" layim-event="msgVoice" title="提示音设置" > &#xe645;</li>'
      ,'</ul>'
  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
    ,'<div class="layui-unselect layim-chat-title">'
      ,'<div class="layim-chat-other">'
        ,'<img class="layim-{{ d.data.type }}{{ d.data.id }}" src="{{ d.data.avatar }}"><span class="layim-chat-username  layim-{{ d.data.id }}" layim-event="{{ d.data.type==="group" ? \"groupMembers\" : \"\" }}">{{ d.data.name||"佚名" }} {{d.data.temporary ? "<cite>临时会话</cite>" : ""}} {{# if(d.data.type==="group"){ }} <em class="layim-chat-members"></em><i class="layui-icon">&#xe61a;</i> {{# } }}</span>'
    ...
    ,'<div class="layim-chat-footer">'
      ,'<div class="layui-unselect layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
        ...
        ,'{{# }; }}'
      ,'</div>'
      ,'<div class="layim-chat-textarea"><textarea id="chatContent"></textarea></div>'
  ...
  var layimMain, popim = function(content){
	var height = (document.body.clientHeight<520) ? document.body.clientHeight:520;
    return layer.open({
      type: 1
      ,area: ['260px', height+'px']
      ...
      ,success: function(layero){
    	 if(height <520){
    		 ...
    		 $(".layim-tab-content").css("height",h+"px");
    	}
  ...
  var banRightMenu = function(){
    ...
    layimMain.find('.layim-list-group').on('contextmenu', ' li', function (e) {
    	 var othis = $(this);
         var id = othis[0].className.substring(11);
         var name = othis[0].childNodes[1].innerHTML;

         var html = '<ul id="contextmenu_' + id + '" data-id="' + id + '" data-index="' + othis.data('index') + '">';
         html += '<li  id="groupInfor_' + id+ '"layim-event="group_infor"><i class="layui-icon" >&#xe611;</i>&nbsp;&nbsp;查看群资料</li>';
         html += '<li  id="dissolve_' + id+'_' +name+'" name="'+name+'" layim-event="dissolve"><i class="layui-icon">&#x1006</i>&nbsp;&nbsp;解散群</li>';
         html += '</ul>';
  ...
  var layimClose, popmin = function(content){
    ...
    return layer.open({
      ...
      ,success: function(layero, index){
    	  //draggable拖拽，只允许按头像可以拖动
    	var layerId = $(".layui-layim-min,.layui-layim-close").attr("id");
    	$( "#"+layerId ).draggable({ containment: "body", handle: "img" });;
        layimClose = layero;
        if(cache.base.right){
          layero.css('margin-left', '-' + cache.base.right);
        }
        var height = document.body.clientHeight /2;
        layero.css('top', height+'px');
        layero.on('click', function(){
          layer.close(index);         
          if(loginType == "2"){
        	  var height = (document.body.clientHeight<520) ? document.body.clientHeight:520;
              $(".layui-layim").css("height",height+"px");             
              var h = height-171;
              $(".layim-tab-content").css("height",h+"px");
          }
  ...
  var layimChat, layimMin, chatIndex, To = {}, popchat = function(data){
    ...
    if(chat[0]){
      ...
      if(list.find('li').length === 1 && !listThat[0]){
    	  if(loginType == "2"){
    		  chatBox.css('margin-left', '150px');
    		  $(".layim-chat-list").css("width","150px")
    	  }else{
    ...
    if(loginType == "2"){
    	width = width*0.8;
    	$("body .layui-layim-chat").css({"overflow": "scroll"});
    	ifmax=false;
    ...
    var index = chatIndex = layer.open({
      ...
      ,cancel: function(index, layero){ 
    	  thatChat = thisChat();
    	  if(thatChat.data.type == "group"){
    		  var id = thatChat.data.id;
    		  var close_time = new Date().getTime();
   		         groupColseTime[id] = close_time;
   		         userGroupClose[localuser] = groupColseTime;
   		        localStorage.setItem("userGroupClose",JSON.stringify( userGroupClose));              
    	  }
    ...
    setChatMin.index = layer.open({
      ...
      ,success: function(layero, index){
        ...
        layero.find('.layui-layer-content span').on('click', function(){
          layer.close(index);
          layui.data('layim')[cache.mine.id]
  ...
  var messageNew = {}, getMessage = function(data){
    ...
    if(thatChat.data.type + thatChat.data.id !== data.type + data.id){   	
         ...
         //诺打开的不是当前的窗口，则闪烁
         var o = ".layim-chatlist-"+data.type + data.id+" img";
         if( data.id in shakeMap) { //判断是否存在         
	    }else{
	    	var  shake= setInterval("shake('"+o+"');",500);
	         shakeMap[data.id] = shake;
	    }
  ...
  var pushChatlog = function(message){
   ...
   // console.log(layui.data('layim')[cache.mine.id]);
  };
  ...
  var anim = 'layui-anim-upbit', events = {
    ...
    ,msgVoice :function (){//设置是否关闭提示音
    
    	var eleMsgVoice =	[
			'<form class="layui-form" action="">',
			'<div class="layui-form-item">',
			  '<label class="layui-form-label">信息提示音</label>',
			  '<div class="layui-input-block">',
			    '<input id="inforVoice" checked="" name="open" lay-skin="switch" lay-text="ON|OFF"  lay-filter="switchTest"    type="checkbox">',
			  '</div>',
			'</div>',
			'</from>'
               ].join('');
            ...
            ,success: function(layero, index){
            	layui.use('form', function(){            		  
            		  if(layui.layim.cache().base.voice == false){
            			  $("#inforVoice").attr("checked",false );
            		  }else{
            			  $("#inforVoice").attr("checked","checked" );
            		  }
    ...
    //查看群资料
    ,group_infor :function(othis){
    	var id = othis[0].id;
	    groupId = id.split("_")[1];
	    groupId = parseInt(groupId);
    	var elemAddTpl =[
				'<div class="newGroupChat" style="width:400px;height:233px;">',
				...
				'<form class="layui-form" action="">',
				  '<div class="layui-form-item">',
				    ...
				    '<div class="layui-input-block">',
				      '<input id="groupMaster" readonly="readonly" placeholder="请输入群组" class="layui-input" style="width:80%" type="text">',
				    '</div>',
				  '</div>',
				  '<div class="layui-form-item">',
				    '<label class="layui-form-label">群名称</label>',
				    '<div class="layui-input-block">',
				      '<input  id="groupName" readonly="readonly"  placeholder="请输入群名称" class="layui-input" style="width:80%" type="text">',
				    '</div>',
				  '</div>',
				  '<div class="layui-form-item">',
				    '<label class="layui-form-label">群简介</label>',
				    '<div class="layui-input-block">',
				      '<input id="groupContent"  readonly="readonly" placeholder="请输入群简介" class="layui-input" style="width:80%"  type="text">',
				    '</div>',
				  '</div>',
				 '</form>',
				'</div>'].join('');
		layui.layer.open({
			  ...
			  success: function(layero, index){//调用后台查询群资料	
				  var _msg ={
		    				"parameterList" : [ {
		    					"groupId" : groupId		    					
		    				}],
		    				"sysMap" : {
		    					"oper_type" : "selectInfor"
		    				}
		    		};
		    		var data = $.fn.commonPlugin.commonAjax("chatGroupController.do",$.toJSON(_msg));
		    		if (data.retCode = "000000"){
		    			$.each(data.retMap.list, function(index, value) {	
		    				$("#groupMaster").val(value.masterNo);
		    				if(value.groupContent == null){
		    					$("#groupContent").val("无");
		    				}else{
		    					$("#groupContent").val(value.groupContent);
		    				}		    				
    ...
    //解散群
    ,dissolve:function (othis){
    	var id = othis[0].id;
    	    groupId = id.split("_")[1];
    	    groupName = id.split("_")[2];
    	    groupId = parseInt(groupId);
       if( masterMap[groupId] == localuser){
    	   var _msg ={
    				"parameterList" : [ {
    					"groupId":groupId,
    					 "groupName":groupName
    				...
    				"sysMap" : {
    					"oper_type":"deleteGroup"
    				}
    		};
    		var data = $.fn.commonPlugin.commonAjax("chatGroupController.do",$.toJSON(_msg));
       }else{
    	   layer.msg('不是管理员不能进行此操作');
       }
    }
    ...
    ,closeChat: function(othis, e){
    	  //获取群组关闭的时间，保存到本地的localstroage
    	   var id = othis.parent()[0].classList[0];
    	   if(id.substring(0,11) ==  "layim-group"){
    		   id = id.substring(11);
    		   var close_time = new Date().getTime();
    		   groupColseTime[id] = close_time;
    		   userGroupClose[localuser] = groupColseTime;
    ...
    ,groupMembers: function(othis, e){
      ...
      if(othis.data('down')){
        ...
        events.groupMembers.index = layer.tips('<ul id="groupMember" class="layim-members-list"><li id="addMember"><a href="javascript:;"><img src="images/IF/chat/addgroup.png"><cite>添加成员</cite></a></li></ul>', othis, {
          tips: 3
          ,time: 0
          ,anim: 5
          ,fixed: true
          ...
          ,success: function(layero){
            ...
            var _msg ={
        			"parameterList" : [ {"groupId":thatChat.data.id}  ],
        			"sysMap" : {
        				"oper_type":"showMembers"
        			}
        	};
            var data = $.fn.commonPlugin.commonAjax("chatGroupController.do",$.toJSON(_msg));
            var memberArr=data.retMap.member;
                memberArr=memberArr.split(",");
                group[thatChat.data.id]=data.retMap.member;
                var li="";
            for(var i=0;i<memberArr.length;i++){           	
            	 var item={
            		   "username": memberArr[i] //群员昵称
            		   ,"id": memberArr[i] //群员id
            		  ,"avatar": "images/IF/chat/user.png" //群员头像
            	 }
            	 li += '<li data-uid="'+ memberArr[i]+'"><a href="javascript:;"><img src="images/IF/chat/user.png"><cite>'+ memberArr[i] +'</cite></a></li>';    
            }
            li += '<li id="deleteMember"><a href="javascript:;"><img src="images/IF/chat/deleteUser.png"><cite>删除成员</cite></a></li>';
            $("#groupMember").append(li);
            othis.find('.layim-chat-members').html((memberArr||[]).length + '人');         
    ...
    ,chatLog: function(othis){
      ...
      return events.chatLog.index = layer.open({
        type: 2
        ,maxmin: false
        ,title:( thatChat.data.type == "friend" )? ('与 '+ thatChat.data.name +' 的聊天记录'):(thatChat.data.name +'群的聊天记录')
        ,area: ['500px', '100%']
        ,shade: false
        ,offset: 'rb'
        ,skin: 'layui-box'
    ...
    ,menuHistory: function(othis, e){
      ...
      if(type === 'one'){
    	var historyClass = $(".layim-list-history");
		historyClass.find('>li.layim-'+parent.data('index')).remove(); 
 ...
 function add_member(id,member,groupName,isOk,masterNo){//已经在群组中的成员不能再勾选
	var elemAddTpl =[
						'<div class="newGroupChat" style="width:400px;height:400px;">',	   
						    '<div class="right" style="width:100%;float:right; height:100%;">',
						      '<div style="width:90%;height:100%;float:right;margin-right:3px">',
						          '<fieldset style="height:98%;" >',
						             '<legend>群成员设置</legend>',
						             '<div style="overflow:auto;width:90%;height:85%;" id="selectMember">',
						 			   '<ul id="selectMemberTree" class="ztree" data-faicon="user"></ul>',
						 		     '</div>',
						          '</fieldset>', 
						        '</div>',  
						    '</div>',
 	...
 	layui.layer.open({
		  ...
		  ,content:elemAddTpl,
		  skin: 'layui-layer-molv',
		  btn: true ? ['确认', '取消'] : ['发送申请', '关闭'],
		  yes: function(index, layero){//确定按钮点击事件
			  addMember(index,id,member,groupName,isOk,masterNo);				  
		  }
		});  
    	getTree(treeArr,"selectMemberTree");
    	var memberArr = member.split(",");
    	var treeObj = $.fn.zTree.getZTreeObj("selectMemberTree");
    	for (var i=0; i < memberArr.length ; i++) {
    		var node = treeObj.getNodeByParam("id", memberArr[i], null); 		
    		treeObj.checkNode(node, true, true);
    		treeObj.setChkDisabled(node, true);
    	}
    }
function addMember(index,id,member,groupName,isOk,masterNo){
	var selectNode = getTreeSelectNodes("selectMemberTree");
	members=selectNode+","+member;
	var _msg ={
			"parameterList" : [ {
				"groupId":id,
				"groupMember":members,
			...
			"sysMap" : {
				"oper_type":"UPDATE",
				"update_type":"add_"+selectNode
			}
	};
	var data = $.fn.commonPlugin.commonAjax("chatGroupController.do",$.toJSON(_msg));
	var membersArr = members.split(",");
	if (data.retCode = "000000"){
		layui.layer.close(index);//关闭页面	
		$('.layim-'+id+' .layim-chat-members').html(membersArr.length +'人'); 
	}
}
function delete_member(id,member,groupName){
	var elemAddTpl =[
						'<div class="newGroupChat" style="width:400px;height:400px;">',	   
						    '<div class="right" style="width:100%;float:right; height:100%;">',
						      '<div style="width:90%;height:100%;float:right;margin-right:3px">',
						          '<fieldset style="height:98%;" >',
						             '<legend>请勾选需要删除人员</legend>',
						             '<div style="overflow:auto;width:90%;height:85%;" id="selectMember">',
						 			   '<ul id="selectMemberTree" class="ztree" data-faicon="user"></ul>',
						 		     '</div>',
						          '</fieldset>', 
 	...
 	layui.layer.open({
		  ...
		  btn: true ? ['确认', '取消'] : ['发送申请', '关闭'],
		  yes: function(index, layero){//确定按钮点击事件
			  deleteMember(index,id,member,groupName);				  
		  }
		});  
    	getTree(treeArr,"selectMemberTree");
    	var memberArr = member.split(",");
    	...
    	var nodes =  treeObj.getNodeByParam("id", localuser, null);
    	var parentnode = nodes.getParentNode();
    	nodes.checked= false;
    	parentnode.checked= false
    	treeObj.updateNode(nodes,true);
    	treeObj.updateNode(parentnode,true);
    }
function  deleteMember(index,id,member,groupName){
	var resulet =  0;
	var selectNode = getTreeSelectNodes("selectMemberTree");
	selectArr = selectNode.split(",");
	for(var i=0 ;i<selectArr.length;i++){
		if(selectArr[i] == masterMap[id]){
			resulet = 2;
			break;
		}
		if(selectArr[i] == localuser){
			resulet = 1;
		}
	}
	if(resulet == 2){
		layui.layer.msg("管理员不能被删除");
		return false;
	}
	var memberArr = member.split(",");
	for(var i=0;i<memberArr.length;i++){
		for(var k=0;k<selectArr.length;k++){
			if(memberArr[i]==selectArr[k]){
				memberArr[i]="";
				}
		}
	}
	...
	var _msg ={
			...
			"sysMap" : {
				"oper_type":"UPDATE",
				"update_type":"deletegroup_"+selectNode
			}
	};
	var data = $.fn.commonPlugin.commonAjax("chatGroupController.do",$.toJSON(_msg));
	var membersArr = members.split(",");
...
//获取选中的节点
function getTreeSelectNodes(treeid){
	var treeObj = $.fn.zTree.getZTreeObj(treeid);
	var nodes = treeObj.getCheckedNodes(true);
	var selectNode="";
for (var i=0, l=nodes.length; i < l; i++) {
	if(nodes[i].id.indexOf("-")==-1){
		selectNode = selectNode + "," + nodes[i].id;
	}
}
if (selectNode != ""){
...
//构造树
function getTree(zTreeNodes,treeid) {
	setting = {
			view : {
   ...
   var zTreeObj = $.fn.zTree.init($("#"+treeid), setting, zTreeNodes);	 	 
}
function shake(o){
